...
func (c *Cluster) reconcileMonPDB() (*cephclient.MonStatusResponse, error) {
	...
	if err != nil {
		logger.Warningf("failed to get current mon pdb maxunavailable, proceeding with update. %v", err)
	} else if currentMaxUnavailable == allowedDown {
		logger.Debugf("mon pdb maxunavailable is already set to %d", allowedDown)
		return &quorumStatus, nil
	}

	// update the mon pdb since the maxunavailable changed
	logger.Infof("setting mon pdb maxUnavailable=%d (%d mons down)", allowedDown, downMonCount)
...
func (c *Cluster) getExistingMaxUnavailable() (int32, error) {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			logger.Debugf("mon pdb %q not found", monPDBName)
			return -1, nil
		}
		return 0, errors.Wrapf(err, "failed to get mon pdb %q", existingPDB.Name)
	}
	logger.Debugf("existing mon pdb maxUnavailable=%d", existingPDB.Spec.MaxUnavailable.IntVal)
